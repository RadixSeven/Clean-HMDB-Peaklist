...
# ID number followed by a tab-separated list of the ppms of the peaks
# in the first "Table of Peaks" entry in the HNMR section of the file.
# The printed ppms are followed by a newline.  If there is no table of
...
FILE: foreach my $file (@ARGV){
	...
	if    ($table_state == NOT_IN_HNMR){
		...
		my @fields = split(qr/$sep/);
		my $f = $fields[$idx_ppm]; 
		#Trim whitespace from the field - I don't convert to a
